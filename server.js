...
function renderResultSheetPDF(sheet, res) {
  ...
  const HEADER_H = 66, DIVIDER_H = 4, INFO_H = 84, TABLE_HEAD_H = 20, GAP = 8;
  ...
  // INFO BOX — student details arranged in matched rows: label on top,
  // bold value directly beneath it, left column and right column sharing
  // the same row baseline, with a dotted rule under each row (mirrors the
  // reference report-card screenshot layout).
  ...
  const infoPadX = 14, infoPadTop = 10, rowH = (INFO_H - infoPadTop - 6) / infoLeft.length;
  for (let r = 0; r < infoLeft.length; r++) {
    const rowTop = y + infoPadTop + r * rowH;
    [[infoLeft[r], 0], [infoRight[r], 1]].forEach(([[label, value], colIdx]) => {
      const colX = PAGE_MARGIN + infoPadX + colIdx * colW;
      doc.fillColor(INK_SOFT).font('Helvetica-Bold').fontSize(6.5).text(label, colX, rowTop, { width: colW - infoPadX - 6 });
      doc.fillColor(NAVY).font('Helvetica-Bold').fontSize(9.5).text(value, colX, rowTop + 9, { width: colW - infoPadX - 6 });
    });
    if (r < infoLeft.length - 1) {
      const lineY = rowTop + rowH - 4;
      doc.save().dash(1, { space: 1.5 }).moveTo(PAGE_MARGIN + infoPadX, lineY).lineTo(PAGE_MARGIN + contentWidth - infoPadX, lineY).strokeColor('#E7DFC9').lineWidth(0.6).stroke().undash().restore();
    }
  }
  y += INFO_H + GAP;
